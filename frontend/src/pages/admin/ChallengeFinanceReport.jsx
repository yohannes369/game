...
export default function ChallengeFinanceReport() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const rate = rateRes.data?.ratePercent;

      setRatePercent(
        rate !== undefined && rate !== null ? String(rate) : ''
      );
    } catch (err) {
      setError(
  ...
  const handleRateSave = async (event) => {
    ...
    if (ratePercent === '' || Number.isNaN(numericRate)) {
      setError(
        t(
          'challenge.invalidCommissionRate',
    ...
    if (numericRate < 0 || numericRate > 100) {
      setError(
        t(
          'challenge.invalidCommissionRateRange',
    ...
    try {
      await api.put('/challenges/admin/commission', {
        ratePercent: numericRate,
      });

      setSavedMessage(
        t(
          'challenge.commissionSaved',
          'Commission rate updated. Applies to future draws only.'
        )
      );

      // Reload the report and current commission rate.
      await loadAll();
    } catch (err) {
      setError(
        err.response?.data?.message ||
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-semibold text-gray-900">
          {t(
            'challenge.financeReportTitle',
            'Challenge Finance Report'
          )}
        </h1>

        <p className="mt-2 text-sm text-gray-600">
          {t(
            'challenge.financeReportSubtitle',
            'Every paid-out challenge, the commission taken, and the net amount sent to the winner.'
          )}
        </p>
      </div>
      ...
      <div className="mb-8 rounded-3xl border border-indigo-200 bg-indigo-50 p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-indigo-700">
          {t(
            'challenge.commissionHint',
            'Percentage taken from the total pot before a winner is paid out. Changing this only affects challenges drawn after the change.'
          )}
        </p>
          ...
          <div className="mb-6 grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              <p className="text-sm text-gray-500">
                {t(
                  'challenge.totalPot',
                  'Total Pot Across All Paid'
                )}
              </p>

              <p className="mt-2 text-2xl font-semibold text-gray-900">
                {totals.totalPot ?? 0} Birr
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              <p className="text-sm text-gray-500">
                {t(
                  'challenge.totalCommission',
                  'Total Commission Earned'
                )}
              </p>

              <p className="mt-2 text-2xl font-semibold text-emerald-700">
                {totals.totalCommission ?? 0} Birr
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              <p className="text-sm text-gray-500">
                {t(
                  'challenge.totalPaidOut',
                  'Total Paid Out to Winners'
                )}
              </p>

              <p className="mt-2 text-2xl font-semibold text-gray-900">
                {totals.totalPaidOut ?? 0} Birr
              </p>
            </div>
          </div>
                ...
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.id', 'Challenge')}
                    </th>

                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.winner', 'Winner')}
                    </th>

                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.totalPot', 'Total Pot')}
                    </th>

                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.rate', 'Rate')}
                    </th>

                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.commission', 'Commission')}
                    </th>

                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.payoutNet', 'Net Payout')}
                    </th>

                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                      {t('challenge.paidAt', 'Paid At')}
                    </th>
                  </tr>
                </thead>
                ...
                <tbody className="divide-y divide-gray-100">
                  {challenges.map((row) => (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-emerald-700">
                        {row.commissionAmount ?? 0} Birr
                      </td>

                      {/* Net Payout */}
                      <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-900">
                        {row.payoutNetAmount ?? 0} Birr
                      </td>
                  ...
                  {challenges.length === 0 && (
                    <tr>
                      ...
                      >
                        {t(
                          'challenge.financeReportEmpty',
                          'No paid challenges yet.'
                        )}
                      </td>
                    </tr>
                  )}
