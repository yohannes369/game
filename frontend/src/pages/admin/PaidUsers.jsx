...
export default function PaidUsers() {
  ...
  async function handlePayout(user) {
    ...
    if (!challengeId) {
      setError('Challenge ID is missing for this user. The paid-users API must return challengeId.');
      return;
    }

    const confirmed = window.confirm(
      `Mark payout as paid for ${user.name || user.username || 'this user'}?`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setSuccess(`Payout processed for ${user.name || user.username || 'user'}.`);
      await load();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to process the payout.');
    } finally {
      setProcessingId(null);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {t('paidUsers.title', 'Paid Users')}
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            {t('paidUsers.subtitle', 'All users with at least one approved payment.')}
          </p>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={handleSearchSubmit} className="mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('paidUsers.search', 'Search by name, username or phone')}
            </label>
            <input
              type="text"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder={t('paidUsers.searchPlaceholder', 'e.g. Abebe or 0911…')}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
            />
          </div>
          <div className="flex items-end">
      ...
      <div className="bg-white rounded-lg shadow">
        {loading ? (
          <div className="flex items-center justify-center p-12">
            ...
            <p className="ml-3 text-sm text-gray-500">{t('common.loading', 'Loading…')}</p>
          </div>
        ) : users.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="mt-2 text-sm text-gray-400">{t('paidUsers.empty', 'No paid users found.')}</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {['Name', 'Username', 'Phone', 'Total Paid', 'Payments', 'Tickets', 'Last Payment', 'Action'].map((col) => (
                    <th
                      key={col}
                      className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                    >
                      {t(`paidUsers.${col.toLowerCase().replace(' ', '')}`, col)}
                    </th>
                  ))}
                </tr>
              </thead>
                    ...
                    <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button
                          ...
                          disabled={processingId === u.id || !challengeId}
                          onClick={() => handlePayout(u)}
                          className="inline-flex items-center px-3 py-1.5 bg-green-600 text-white text-xs font-semibold rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
                          {processingId === u.id ? (
                            <>
                              <svg className="animate-spin h-3 w-3 mr-1.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
