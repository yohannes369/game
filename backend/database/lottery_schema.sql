...
CREATE TABLE `challenges` (
  ...
  `amount` decimal(12,2) NOT NULL,
  `status` enum('WAITING','ACCEPTED','PAYMENT_PENDING','ADMIN_REVIEW','APPROVED','NUMBERS_ASSIGNED','DRAW_SCHEDULED','DRAW_COMPLETED','WINNER_REQUESTED_PAYOUT','PAYOUT_REVIEW','PAID','CANCELLED') NOT NULL DEFAULT 'WAITING',
  `payment_reference_creator` varchar(255) DEFAULT NULL,
  `screenshot_creator` varchar(255) DEFAULT NULL,
  `payment_status_creator` enum('PENDING','APPROVED','REJECTED') NOT NULL DEFAULT 'PENDING',
  `payment_rejection_reason_creator` varchar(255) DEFAULT NULL,
  `payment_reference_challenger` varchar(255) DEFAULT NULL,
  `screenshot_challenger` varchar(255) DEFAULT NULL,
  `payment_status_challenger` enum('PENDING','APPROVED','REJECTED') NOT NULL DEFAULT 'PENDING',
  `payment_rejection_reason_challenger` varchar(255) DEFAULT NULL,
  `approved_by` int(11) DEFAULT NULL,
  `approved_at` datetime DEFAULT NULL,
  `random_seed` varchar(255) DEFAULT NULL,
  `draw_at` datetime DEFAULT NULL,
  `winner_user_id` int(11) DEFAULT NULL,
  `winner_ticket_number` varchar(100) DEFAULT NULL,
  `payout_requested_at` datetime DEFAULT NULL,
  `payout_status` enum('waiting_payment','paid','rejected') DEFAULT NULL,
  `payout_transaction_id` varchar(100) DEFAULT NULL,
  `payout_screenshot_path` varchar(255) DEFAULT NULL,
  `payout_approved_by` int(11) DEFAULT NULL,
  `payout_approved_at` datetime DEFAULT NULL,
  `payout_rejection_reason` varchar(255) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `sender_name_creator` varchar(255) DEFAULT NULL,
  `phone_creator` varchar(50) DEFAULT NULL,
  `sender_name_opponent` varchar(255) DEFAULT NULL,
  `phone_opponent` varchar(50) DEFAULT NULL,
  `screenshot_opponent` varchar(255) DEFAULT NULL,
  `sender_name_challenger` varchar(150) DEFAULT NULL,
  `phone_challenger` varchar(50) DEFAULT NULL,
  `ticket_number_creator` varchar(100) DEFAULT NULL,
  `ticket_number_challenger` varchar(100) DEFAULT NULL,
  `commission_rate_percent` decimal(5,2) DEFAULT NULL,
  `commission_amount` decimal(12,2) DEFAULT NULL,
  `payout_net_amount` decimal(12,2) DEFAULT NULL,
  `bank_name` varchar(100) DEFAULT NULL,
  `account_number` varchar(50) DEFAULT NULL,
  `account_name` varchar(150) DEFAULT NULL,
  `creator_payment_submitted_at` datetime DEFAULT NULL,
  `challenger_payment_submitted_at` datetime DEFAULT NULL
...
ALTER TABLE `challenges`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uq_challenges_challenge_id` (`challenge_id`),
  ADD KEY `fk_challenges_approved_by` (`approved_by`),
  ADD KEY `fk_challenges_payout_approved_by` (`payout_approved_by`),
  ...
  ADD KEY `idx_challenges_winner` (`winner_user_id`),
  ADD KEY `idx_challenges_draw_at` (`draw_at`),
  ADD KEY `idx_challenges_payment_status` (`payment_status_creator`,`payment_status_challenger`);
...
--
ALTER TABLE `challenges`
  ADD CONSTRAINT `fk_challenges_approved_by` FOREIGN KEY (`approved_by`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_challenges_challenger` FOREIGN KEY (`challenger_id`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_challenges_creator` FOREIGN KEY (`creator_id`) REFERENCES `users` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_challenges_payout_approved_by` FOREIGN KEY (`payout_approved_by`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_challenges_winner` FOREIGN KEY (`winner_user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
COMMIT;
